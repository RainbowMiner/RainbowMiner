[PSCustomObject]@{
        ...
        "MiningRigRentals" = [PSCustomObject]@{
            Fields=[PSCustomObject]@{User="";API_Key="";API_Secret="";EnableMining="0";EnableAutoCreate="1";EnablePriceUpdates="1";EnableAutoPrice="1";PriceBTC="";PriceOffset="30";Title = "%algorithm% mining";Description = "Autostart mining with RainbowMiner (https://rbminer.net) on $(if ($IsWindows) {"Windows"} else {"Linux"}). This rig is idle and will activate itself, as soon, as you rent it. [%workername%]"}
            SetupFields=[PSCustomObject]@{User="Enter your MiningRigRentals username";API_Key="Enter your MiningRigRentals API key";API_Secret = "Enter your MiningRigRentals API secret key";EnableAutoCreate="Automatically create MRR-rigs";EnablePriceUpdates="Enable rental price updates";EnableAutoPrice="Enable automatic price changes to rig's profit";PriceBTC="Fixed price in BTC (used, if EnableAutoPrice=0)";PriceOffset="Add value to be added to auto price (in %)";EnableMining="Enable switching to MiningRigRentals, even it is not rentend (not recommended)";Title="Title for autocreate, %algorithm% will be substituted with algorithm";Description="Description for autocreate, %workername% will be substituted with rig's workername"}
            Currencies=@()
            Autoexchange=$true
        }
