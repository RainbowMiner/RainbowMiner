[PSCustomObject]@{
        ...
        "MiningRigRentals" = [PSCustomObject]@{
            Fields=[PSCustomObject]@{User="";API_Key="";API_Secret="";EnableMining="0";EnableAutoCreate="0";AutoCreateMinProfitPercent="50";AutoCreateMinCPUProfitBTC="0.00001";AutoCreateMaxMinHours="24";EnableAutoUpdate="0";EnableAutoPrice="1";EnableMinimumPrice="1";AutoPriceModifierPercent="0";PriceBTC="0";PriceFactor="1.75";MinHours="3";MaxHours="168";PriceCurrencies="BTC";Title = "%algorithm% mining";Description = "Autostart mining with RainbowMiner (https://rbminer.net) on $(if ($IsWindows) {"Windows"} else {"Linux"}). This rig is idle and will activate itself, as soon, as you rent it. %workername%"}
            SetupFields=[PSCustomObject]@{User="Enter your MiningRigRentals username";API_Key="Enter your MiningRigRentals API key";API_Secret = "Enter your MiningRigRentals API secret key";EnableAutoCreate="Automatically create MRR-rigs";EnableAutoUpdates="Automatically update MRR-rigs";AutoCreateMinProfitPercent="Enter minimum profitability in percent compared to current best profit, for full rigs to be autocreated on MRR";AutoCreateMinCPUProfitBTC="Enter minimum one-day revenue in BTC, for a CPU-only rig to be autocreated on MRR";AutoCreateMaxMinHours="Enter the maximum hours for minimum rental time, for a rig to be autocreated on MRR";EnableAutoPrice="Enable MRR automatic prices";EnableMinimumPrice="Set MRR automatic minimum price";AutoPriceModifierPercent="Autoprice modifier in percent (e.g. +10 will increase all suggested prices by 10%)";PriceBTC="Fixed price in BTC (used, if EnableAutoPrice=0)";PriceFactor="Enter profit multiplicator: price = rig's profit x this multiplicator";PriceCurrencies="List of accepted currencies (must contain BTC)";MinHours="Minimum rental time in hours (min. 3)";MaxHours="Maximum rental time in hours (min. 3)";EnableMining="Enable switching to MiningRigRentals, even it is not rentend (not recommended)";Title="Title for autocreate, %algorithm% will be substituted with algorithm";Description="Description for autocreate, %workername% will be substituted with rig's workername"}
            Currencies=@()
            Autoexchange=$true
        }
