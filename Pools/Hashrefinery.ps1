...
try {
    $Pool_Request = Invoke-RestMethodAsync "http://pool.hashrefinery.com/api/status" -tag $Name -cycletime 120
}
catch {
...
try {
    $PoolCoins_Request = Invoke-RestMethodAsync "http://pool.hashrefinery.com/api/currencies" -tag $Name -cycletime 120
}
catch {
    if ($Error.Count){$Error.RemoveAt(0)}
