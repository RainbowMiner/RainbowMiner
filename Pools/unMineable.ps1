...
$Pools_Data | ForEach-Object {
    ...
    foreach($Pool_CurrencyData in $Pool_Currencies) {
        ...
        if (-not $InfoOnly) {
            ...
            try {
                $Pool_ProfitRequest = Invoke-RestMethodAsync "https://api.unminable.com/v3/calculate/reward" -tag $Name -cycletime 120 -body @{algo=$Pool_RewardAlgo;coin=$Pool_Currency;mh=100}                
            } catch {
                if ($Error.Count){$Error.RemoveAt(0)}
