...
$Success = $true
try {
    if (-not ($Pool_Request = Invoke-RestMethodAsync "https://ravenminer.com/api/status" -tag $Name -cycletime 120)){throw}
}
catch {
...
try {
    if (-not ($PoolCoins_Request = Invoke-RestMethodAsync "https://ravenminer.com/api/currencies" -tag $Name -cycletime 120)){throw}
}
catch {
    if ($Error.Count){$Error.RemoveAt(0)}
    $Success = $false
