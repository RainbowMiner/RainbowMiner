...
$PoolCoins_Request.PSObject.Properties.Name | Where-Object {$Wallets.$_ -or $InfoOnly} | Foreach-Object {
    ...
    if (-not $InfoOnly) {
        ...
        $Stat = Set-Stat -Name "$($Name)_$($Pool_CoinSymbol)_Profit" -Value $Price_BTC -Duration $(if ($NewStat) {New-TimeSpan -Days 1} else {$StatSpan}) -ChangeDetection $true -Difficulty $difficulty -Quiet
