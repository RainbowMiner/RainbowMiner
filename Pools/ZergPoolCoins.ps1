...
$PoolCoins_Request | Get-Member -MemberType NoteProperty -ErrorAction Ignore | Select-Object -ExpandProperty Name | ForEach-Object {
    ...
    $Pool_TSL = if ($PoolCoins_Request.$_.timesincelast_shared -ne $null) {$PoolCoins_Request.$_.timesincelast_shared} else {$PoolCoins_Request.$_.timesincelast}
    ...
    if (-not $InfoOnly) {
        if ($Pool_Request.$Pool_Algorithm.coins -eq 1) {
            ...
            $Pool_Actual24h   = $PoolCoins_Request.$_.actual_last24h_shared/1000
            $Pool_Estimate24h = $PoolCoins_Request.$_.estimate_last24
        }
        $Stat = Set-Stat -Name "$($Name)_$($_)_Profit" -Value ([Double]$PoolCoins_Request.$_.estimate / $Divisor) -Duration $StatSpan -ChangeDetection $true -Actual24h $Pool_Actual24h -Estimate24h $Pool_Estimate24h -HashRate $PoolCoins_Request.$_.hashrate_shared -BlockRate $PoolCoins_Request.$_."24h_blocks_shared" -Quiet
    ...
    if (($Pool_ExCurrency -and $Wallets.$Pool_ExCurrency) -or $InfoOnly) {
        ...
        foreach($Pool_Region in $Pool_RegionsTable.Keys) {
            ...
            [PSCustomObject]@{
                ...
                Workers       = $PoolCoins_Request.$_.workers_shared
