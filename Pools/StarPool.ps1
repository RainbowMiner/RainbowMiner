...
try {
    $Pool_Request = Invoke-RestMethodAsync "https://www.starpool.biz/api/status" -tag $Name
}
catch {
...
try {
    $PoolCoins_Request = Invoke-RestMethodAsync "https://www.starpool.biz/api/currencies" -tag $Name
}
catch {
    if ($Error.Count){$Error.RemoveAt(0)}
...
$Pool_Currencies = @("BTC","DASH","LTC","CANN","DGB") + @($Wallets.PSObject.Properties.Name | Select-Object) + @($PoolCoins_Request | Get-Member -MemberType NoteProperty -ErrorAction Ignore | Select-Object -ExpandProperty Name | Foreach-Object {if ($PoolCoins_Request.$_.symbol -eq $null){$_} else {$PoolCoins_Request.$_.symbol}}) | Select-Object -Unique | Where-Object {$Wallets.$_ -or $InfoOnly}
...
$Pool_Request | Get-Member -MemberType NoteProperty -ErrorAction Ignore | Select-Object -ExpandProperty Name | Where-Object {$Pool_Request.$_.hashrate -gt 0} |ForEach-Object {
    ...
    if (-not $InfoOnly) {
        ...
        else {$Stat = Set-Stat -Name "$($Name)_$($Pool_Algorithm_Norm)_Profit" -Value (Get-YiiMPValue $Pool_Request.$_ -DataWindow $DataWindow -Factor $Pool_Factor) -Duration $StatSpan -ChangeDetection $true -HashRate $Pool_Request.$_.hashrate -BlockRate $Pool_BLK}
    }
