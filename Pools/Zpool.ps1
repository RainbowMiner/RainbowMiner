...
try {
    $Pool_Request = Invoke-RestMethodAsync "http://www.zpool.ca/api/status" -tag $Name -cycletime 120
}
catch {
...
try {
    $PoolCoins_Request = Invoke-RestMethodAsync "http://www.zpool.ca/api/currencies" -tag $Name -cycletime 120
}
catch {
    if ($Error.Count){$Error.RemoveAt(0)}
...
@("na","eu","sea","jp") | Foreach-Object {$Pool_RegionsTable.$_ = Get-Region $_}
...
$Pool_Request | Get-Member -MemberType NoteProperty -ErrorAction Ignore | Select-Object -ExpandProperty Name | Where-Object {$Pool_Request.$_.hashrate -gt 0 -or $InfoOnly -or $AllowZero} | ForEach-Object {
    ...
    if (-not $InfoOnly) {
        ...
        $Stat = Set-Stat -Name "$($Name)_$($Pool_Algorithm_Norm)_Profit" -Value $Pool_Price -Duration $(if ($NewStat) {New-TimeSpan -Days 1} else {$StatSpan}) -ChangeDetection $(-not $NewStat) -Actual24h $($Pool_Request.$_.actual_last24h/1000) -Estimate $($Pool_Request.$_.estimate_current) -HashRate $Pool_Request.$_.hashrate -BlockRate $Pool_BLK -Quiet
    }
