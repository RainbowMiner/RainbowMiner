...
$Pools_Data | Where-Object {$Pool_Algorithms -icontains $_.rpc} | Where-Object {($Wallets."$($_.symbol)" -and (-not $_.symbol2 -or $Wallets."$($_.symbol2)")) -or $InfoOnly} | ForEach-Object {
    ...
    if (-not $InfoOnly) {
        ...
        try {
            ...
            if ($Pool_Currency2) {
                $Pool_Request2 = Invoke-RestMethodAsync "https://cryptoknight.cc/rpc/$($Pools_Data | Where-Object {$_.symbol -eq $Pool_Currency2 -and -not $_.symbol2} | Select-Object -ExpandProperty rpc)/stats" -tag $Name -timeout 15 -cycletime 120
            }
        }
        catch {
            if ($Error.Count){$Error.RemoveAt(0)}
