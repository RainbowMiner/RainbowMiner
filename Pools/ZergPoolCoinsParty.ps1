...
$PoolCoins_Request | Get-Member -MemberType NoteProperty -ErrorAction Ignore | Select-Object -ExpandProperty Name | ForEach-Object {
    ...
    if (-not $InfoOnly) {
        ...
        $Stat = Set-Stat -Name "$($Name)_$($Pool_CoinSymbol)_Profit" -Value ([Double]$PoolCoins_Request.$Pool_CoinSymbol.estimate / $Divisor) -Duration $StatSpan -ChangeDetection $false -Actual24h $Pool_Actual24h -Estimate24h $Pool_Estimate24h -HashRate $PoolCoins_Request.$Pool_CoinSymbol.difficulty -Quiet
    }
    ...
    if (($Pool_ExCurrency -and $Wallets.$Pool_ExCurrency) -or $InfoOnly) {
        ...
        foreach($Pool_Region in $Pool_Regions) {
            ...
            [PSCustomObject]@{
                ...
                Pass          = "ID={workername:$Worker},c=$Pool_ExCurrency,mc=$Pool_Currency,m=party.$($PartyPassword){diff:,sd=`$difficulty}$Pool_Params"
