        ...
        foreach($Pool_SSL in ($false,$true)) {
            ...
            foreach($Pool_Region in $Pool_Regions) {
                ...
                [PSCustomObject]@{
                    ...
                    Pass               = "ID={workername:$Worker},c=$(if ($Pool_ExCurrency -eq "USDT" -and $USDT_Token) {$USDT_Token} else {$Pool_ExCurrency}),mc=$Pool_Currency,m=party.$($PartyPassword){diff:,sd=`$difficulty}$Pool_Params"
