...
$Pool_Request | Where-Object {$Pool_Currency = $_.coin -replace "(29|31)" -replace "^VDS$","VOLLAR";$Wallets.$Pool_Currency -or $Wallets."$($_.coin)" -or $InfoOnly} | ForEach-Object {
            ...
            foreach($Pool_Host in @($_.address -split ',')) {
                if ($Pool_Host -match "^(.+?):(\d+)") {
                    [PSCustomObject]@{
                        ...
                        Pass          = "x"
                        Region        = Switch -Regex ($Pool_Host) {"\(EU\)" {$Pool_Regions.EU};"\(US\)" {$Pool_Regions.US};default {$Pool_Regions.CN}}
                        SSL           = $false
