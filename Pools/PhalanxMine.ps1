...
$PoolCoins_Request.PSObject.Properties.Name | Where-Object {$Wallet.$_ -or $InfoOnly} | ForEach-Object {
    ...
    $Pool_PoolFee = [double]$PoolCoinsRequest.$Pool_CoinSymbol.fees
    ...
    if (-not $InfoOnly) {
        $Stat = Set-Stat -Name "$($Name)_$($Pool_CoinSymbol)_Profit" -Value 0 -Duration $StatSpan -HashRate $PoolCoins_Request.$Pool_CoinSymbol.hashrate_shared -BlockRate $PoolCoins_Request.$Pool_CoinSymbol."24h_blocks_shared" -Difficulty $PoolCoins_Request.$Pool_CoinSymbol.difficulty -ChangeDetection $false -Quiet
