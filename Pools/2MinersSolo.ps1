...
$Pools_Data | Where-Object {$Wallets."$($_.symbol -replace "\d")" -or $InfoOnly} | ForEach-Object {
    ...
    if ($ok) {
        ...
        $Pool_HostStatus | Where-Object {$_.host -match "$($Pool_Host)" -and $Pool_Hosts -notcontains "$($_.host)$([int]$_.port -ge 10000)"} | Select-Object host,port | Sort-Object -Descending:$($Pool_Currency -eq "XZC") {[int]$_.port} | Foreach-Object {
