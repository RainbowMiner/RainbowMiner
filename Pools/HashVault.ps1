...
$Pools_Data = @(
    ...
    [PSCustomObject]@{symbol = "SAL";   port = 3333; fee = 0.9; rpc = "salvium"} #pool.salvium.hashvault.pro:3333
...
$Pools_Data | Where-Object {$Wallets."$($_.symbol)" -or $InfoOnly} | ForEach-Object {
    ...
    if ($ok -and -not $InfoOnly) {
        ...
        $lastBTCPrice = if ($Global:VarCache.Rates.$Pool_Currency) {1/$Global:VarCache.Rates.$Pool_Currency} else {[decimal]$Pool_Request.market.price_btc}
