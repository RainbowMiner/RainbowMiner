...
try {
    $PoolCoins_Request = Invoke-RestMethodAsync "https://www.nlpool.nl/api/currencies" -cycletime ([Math]::Max(120,$Session.Config.Interval)) -delay 1000 -tag $Name
}
catch {
    if ($Error.Count){$Error.RemoveAt(0)}
...
$Pool_Request | Get-Member -MemberType NoteProperty -ErrorAction Ignore | Select-Object -ExpandProperty Name | Where-Object {$Pool_Request.$_.hashrate -gt 0 -or $AllowZero -or $InfoOnly} | ForEach-Object {
    ...
    if (-not $InfoOnly) {
        ...
        $Stat = Set-Stat -Name "$($Name)_$($Pool_Algorithm_Norm)_Profit" -Value ($Pool_Price*$Pool_Divisor) -Duration $(if ($NewStat) {New-TimeSpan -Days 1} else {$StatSpan}) -ChangeDetection $(-not $NewStat) -ErrorRatio 0.000 -HashRate ($Pool_Request.$_.hashrate/$Pool_Divisor) -BlockRate $Pool_BLK -Quiet
    }
