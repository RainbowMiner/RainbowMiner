...
try {
    $Pool_Request = Invoke-RestMethodAsync "http://www.phi-phi-pool.com/api/status" -retry 3 -retrywait 1000 -tag $Name -cycletime 120
}
catch {
...
try {
    $PoolCoins_Request = Invoke-RestMethodAsync "http://www.phi-phi-pool.com/api/currencies" -delay 1000 -tag $Name -cycletime 120
}
catch {
    if ($Error.Count){$Error.RemoveAt(0)}
...
$Pool_Currencies = @("DOGE", "BTC") + @($Wallets.PSObject.Properties.Name | Select-Object) + @($PoolCoins_Request.PSObject.Properties.Name | Foreach-Object {if ($PoolCoins_Request.$_.symbol -eq $null){$_} else {$PoolCoins_Request.$_.symbol}}) | Select-Object -Unique | Where-Object {$Wallets.$_ -or $InfoOnly}
    ...
    foreach($Pool_Region in $Pool_Regions) {
        foreach($Pool_Currency in $Pool_Currencies) {
            ...
            [PSCustomObject]@{
                ...
                Pass          = "c=$Pool_Currency{diff:,sd=`$difficulty}$Pool_Params"
