...
try {
    $Pool_Request = Invoke-RestMethodAsync "https://www.ahashpool.com/api/status" -tag $Name -cycletime 120
}
catch {
...
try {
    $PoolCoins_Request = Invoke-RestMethodAsync "https://www.ahashpool.com/api/currencies" -tag $Name -cycletime 120 -delay 1000
}
catch {
    if ($Error.Count){$Error.RemoveAt(0)}
