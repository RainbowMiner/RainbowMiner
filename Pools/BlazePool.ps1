...
$Pool_Request | Get-Member -MemberType NoteProperty -ErrorAction Ignore | Select-Object -ExpandProperty Name | Where-Object {(($Pool_Request.$_.hashrate -gt 0  -or $AllowZero) -and [Double]$Pool_Request.$_.estimate_current  -gt 0) -or $InfoOnly} | ForEach-Object {
    ...
    if (-not $InfoOnly) {
        ...
        else {$Stat = Set-Stat -Name "$($Name)_$($Pool_Algorithm_Norm)_Profit" -Value (Get-YiiMPValue $Pool_Request.$_ -DataWindow $DataWindow -Factor $Pool_Factorr) -Duration $StatSpan -ChangeDetection $true -HashRate $Pool_Request.$_.hashrate -BlockRate $Pool_BLK -Quiet}
    }
        ...
        foreach($Pool_Currency in $Pool_Currencies) {
            foreach($Pool_Algorithm_Norm in $Pool_Algorithm_All) {
                [PSCustomObject]@{
                    ...
                    Pass          = "ID=$Worker,c=$Pool_Currency{diff:,d=`$difficulty}"
