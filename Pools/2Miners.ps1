...
$Pools_Data = @(
    ...
    [PSCustomObject]@{rpc = "grin";  symbol = "GRIN29";  port = 3030; fee = 1.0; divisor = 1e9; cycles = 21}
...
$Pools_Data | Where-Object {$Wallets."$($_.symbol -replace "\d")" -or $InfoOnly} | ForEach-Object {
    ...
    if ($ok) {
        ...
        $Pool_HostStatus | Where-Object {$_.host -notmatch 'solo-' -and $_.host -match "$($Pool_Host)" -and $Pool_Hosts -notcontains "$($_.host)$($_.port -ge 10000)"} | Select-Object host,port | Foreach-Object {
            $SSL = $_.port -ge 10000
