...
$Pools_Data | Where-Object {$Wallets."$($_.symbol)" -or ($_.altsymbol -and $Wallets."$($_.altsymbol)") -or $InfoOnly} | ForEach-Object {
    ...
    if (-not $InfoOnly) {
        try {
            $Pool_Request = Invoke-RestMethodAsync "https://$($Pool_RpcPath).acepool.top/pool-info.php?miningpoolstats" -tag $Name -timeout 15 -cycletime 120
    ...
    if ($ok -or $InfoOnly) {
        ...
        foreach ($Pool_Region in $Pool_Regions) {
            [PSCustomObject]@{
                ...
                Host          = "$($Pool_HostPath).acepool.top"
                Port          = $Pool_Port
