...
$Pool_Request.pools | Where-Object {$Pool_Currency = $_.coin.type;$Pool_User = $Wallets.$Pool_Currency;$Pool_Currency -eq "AION" -and ($Pool_User -or $InfoOnly)} | Foreach-Object {
    ...
    if (-not $InfoOnly) {
        ...
        $Pool_BlocksRequest = @()
        try {
            $Pool_BlocksRequest = @((Invoke-RestMethodAsync "https://aionmine.org/api/pools/$($Pool_Id)/blocks?pageSize=1000" -tag $Name -retry 3 -retrywait 1000 -timeout 15 -cycletime 120) | Foreach-Object {[PSCustomObject]@{created = Get-Date $_.created;status = $_.status;reward = $_.reward}})
        }
        catch {
            if ($Error.Count){$Error.RemoveAt(0)}
        ...
        if ($ok -and ($Pool_BlocksRequest | Measure-Object).Count) {
            ...
            try {
                $Pool_PerformanceRequest = @((Invoke-RestMethodAsync "https://aionmine.org/api/pools/$($Pool_Id)/performance" -tag $Name -retry 3 -retrywait 1000 -timeout 15 -cycletime 3600).stats | Foreach-Object {[PSCustomObject]@{created = Get-Date $_.created;hashrate = $_.poolHashrate}})
            }
            catch {
                if ($Error.Count){$Error.RemoveAt(0)}
