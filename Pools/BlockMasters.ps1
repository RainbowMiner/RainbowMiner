...
try {
    $Pool_Request = Invoke-RestMethodAsync "http://blockmasters.co/api/status" -retry 3 -retrywait 1000 -tag $Name -cycletime 120
}
catch {
...
try {
    $PoolCoins_Request = Invoke-RestMethodAsync "http://blockmasters.co/api/currencies" -delay 1000 -tag $Name -cycletime 120
}
catch {
    if ($Error.Count){$Error.RemoveAt(0)}
