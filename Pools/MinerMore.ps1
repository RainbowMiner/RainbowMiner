...
try {
    $PoolCoins_Request = Invoke-RestMethodAsync "https://minermore.com/api/currencies" -tag $Name
}
catch {
...
try {
    $Pool_Request = Invoke-RestMethodAsync "https://minermore.com/api/status" -tag $Name
}
catch {
    if ($Error.Count){$Error.RemoveAt(0)}
...
$PoolCoins_Request | Get-Member -MemberType NoteProperty -ErrorAction Ignore | Select-Object -ExpandProperty Name | Where-Object {$PoolCoins_Request.$_.hashrate -gt 0 -or $InfoOnly -or $AllowZero} | ForEach-Object {
    ...
    $Pool_PoolFee = if ($PoolCoins_Request.$Pool_CoinSymbol.fee -ne $null) {$PoolCoins_Request.$Pool_CoinSymbol.fee} else {$Pool_Fee}
