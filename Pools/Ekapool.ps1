...
$OtherCoins | Where-Object {$Pool_Currency = $_.coin;($Wallets.$Pool_Currency -and $Pool_Currency -notin $Pool_Request.pools.coin.symbol -or $InfoOnly)} | Foreach-Object {
    ...
    if ($Pool_LiveStats -and -not $InfoOnly) {
        ...
        if ($Global:Rates.ContainsKey($Pool_Currency)) {
            $Pool_Profit = (86400 / $Pool_LiveStats.network.difficulty) * $Pool_LiveStats.lastblock.reward / $Pool_LiveStats.config.coinUnits * $Global:Rates[$Pool_Currency] / 1e8
        }
