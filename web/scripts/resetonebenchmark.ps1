...
if ($Parameters.name -and $Parameters.algorithm -and $Parameters.DeviceModel) {
	...
	Get-ChildItem ".\Stats\Miners\*-$($Parameters.name)_$($Algorithm)_HashRate.txt" -ErrorAction Ignore | Foreach-Object {
		$count++
		Remove-Item $_ -ErrorAction Ignore
		$text += "$($_.BaseName -replace '-(C|G)PU.+$')/$($DeviceModel)/$($_.BaseName -split '_' | Select-Object -Index 1)`n"
	}
}
