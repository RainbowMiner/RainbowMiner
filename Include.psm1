...
function Get-BalancesContent {
    ...
    $UsePools = @(@(if ($Config.PoolName.Count -gt 0) {$Config.PoolName} else {Get-ChildItem "Pools" -File -ErrorAction Ignore | Select-Object -ExpandProperty BaseName}) | Where-Object {$Config.ExcludePoolName -inotcontains $_} | Select-Object)
    foreach($Balance in @(Get-ChildItem "Balances" -File -ErrorAction Ignore | Where-Object {$UsePools -match "^$($_.BaseName)`(Coins`)?$" -or $Config.ShowPoolBalancesExcludedPools})) {
...
function Set-PoolsConfigDefault {
    ...
    if ($Force -or -not (Test-Path $PathToFile) -or (Get-ChildItem $PathToFile).LastWriteTime.ToUniversalTime() -lt (Get-ChildItem ".\Data\PoolsConfigDefault.ps1").LastWriteTime.ToUniversalTime()) {
        ...
        try {
            ...
            $ChangeTag = Get-ContentDataMD5hash($Preset)
            $Done = [PSCustomObject]@{}
            $Default = [PSCustomObject]@{Penalty = 0;Algorithm = "";ExcludeAlgorithm = "";CoinName = "";ExcludeCoin = "";CoinSymbol = "";ExcludeCoinSymbol = "";FocusWallet = "";AllowZero = "0";EnableAutoCoin = "0";DataWindow="";StatAverage=""}
