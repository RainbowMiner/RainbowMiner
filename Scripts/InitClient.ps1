...
do {
    ...
    if (Get-Yes $Config.EnableServerConfig) {
        $Config.ServerConfigName = Read-HostArray -Prompt "Enter the config files to be copied to this machine" -Default $Config.ServerConfigName -Characters "A-Z" -Valid @("algorithms","coins","config","customminers","miners","mrr","mrralgorithms","ocprofiles","pools","scheduler","userpools")
