...
do {
    ...
    elseif (@("W","C","E","S","A") -contains $SetupType) {
        ...
        Switch ($SetupType) {
            ...
            "C" {$GlobalSetupName = "Common";$GlobalSetupSteps.AddRange(@("miningmode","devicename","cpuminingthreads","cpuminingaffinity","devicenameend","region","currency","uistyle","fastestmineronly","showpoolbalances","showpoolbalancesdetails","showpoolbalancesexcludedpools","showminerwindow","ignorefees","enableocprofiles","enableocvoltage","msia","msiapath","ethpillenable","localapiport","enableautominerports","enableautoupdate")) > $null}
        ...
        do {
            ...
            try {
                Switch ($GlobalSetupSteps[$GlobalSetupStep]) {
                    ...
                    "cpuminingaffinity" {
                        if ($Config.DeviceName -icontains "CPU") {
                            $Config.CPUMiningAffinity = Read-HostString -Prompt "Add --cpu-affinity to CPU miner commandlines (hexadecimal 0x.., leave empty for none)" -Default $Config.CPUMiningAffinity -Characters "A-F0-9x" | Foreach-Object {if (@("cancel","exit","back","<") -icontains $_) {throw $_};$_}
                            if ($Config.CPUMiningAffinity -notmatch "^0x") {$Config.CPUMiningAffinity = "0x$($Config.CPUMiningAffinity)"}
                        } else {
                            $GlobalSetupStepStore = $false
                        }
                    }
