...
$Payout_Currencies = @($Config.Pools.$Name.Wallets.PSObject.Properties.Name | Select-Object Name,Value -Unique | Sort-Object Name,Value)
...
$Payout_Currencies | Foreach-Object {
    try {
        $Request = Invoke-RestMethod "http://api.bsod.pw/api/walletEx?address=$($_.Value)" -UseBasicParsing -TimeoutSec 10 -ErrorAction Stop
