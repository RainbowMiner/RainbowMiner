...
try {
    $PoolCoins_Request = Invoke-RestMethodAsync "https://blockcruncher.com/api/currencies" -tag $Name -cycletime 120
}
catch {
...
$Count = 0
$Payout_Currencies | Where-Object {$PoolCoins_Request.PSObject.Properties.Name -icontains $_.Name} | Foreach-Object {
    try {
        $Request = Invoke-RestMethodAsync "https://blockcruncher.com/api/walletEx?address=$($_.Value)" -delay $(if ($Count){500} else {0}) -cycletime ($Config.BalanceUpdateMinutes*60)
        $Count++
