...
try {
    $PoolCoins_Request = Invoke-RestMethodAsync "http://poolmine.xyz/api/currencies" -tag $Name -cycletime 120 -retry 3 -retrywait 1000
}
catch {
...
$Payout_Currencies | Where-Object {@($PoolCoins_Request.PSObject.Properties | Foreach-Object {if ($_.Value.symbol -ne $null) {$_.Value.symbol} else {$_.Name}} | Select-Object -Unique) -icontains $_.Name -and (-not $Config.ExcludeCoinsymbolBalances.Count -or $Config.ExcludeCoinsymbolBalances -notcontains "$($_.Name)")} | Foreach-Object {
    try {
        $Request = Invoke-RestMethodAsync "http://poolmine.xyz/api/walletEx?address=$($_.Value)" -delay $(if ($Count){500} else {0}) -cycletime ($Config.BalanceUpdateMinutes*60) -retry 3 -retrywait 1000
