...
$Payout_Currencies | Foreach-Object {
    try {
        $Request = Invoke-RestMethodAsync "https://icemining.ca/api/$(if (@("SUQA") -icontains $_.Name) {"wallet"}else{"walletEx"})?address=$($_.Value)" -delay $(if ($Count){500} else {0}) -cycletime ($Config.BalanceUpdateMinutes*60)
        $Count++
