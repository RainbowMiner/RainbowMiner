...
try {
    $PoolCoins_Request = Invoke-RestMethodAsync "http://www.phi-phi-pool.com/api/currencies" -tag $Name -cycletime 120
}
catch {
...
$Count = 0
$Payout_Currencies | Where-Object {@("DOGE", "BTC") + @($PoolCoins_Request.PSObject.Properties | Foreach-Object {if ($_.Value.symbol -ne $null) {$_.Value.symbol} else {$_.Name}} | Select-Object -Unique) -icontains $_.Name} | Where-Object {-not $Config.ExcludeCoinsymbolBalances.Count -or $Config.ExcludeCoinsymbolBalances -notcontains $_} | Foreach-Object {
    try {
        $Request = Invoke-RestMethodAsync "http://www.phi-phi-pool.com/api/walletEx?address=$($_.Value)" -delay $(if ($Count){2000} else {0}) -cycletime ($Config.BalanceUpdateMinutes*60)
        $Count++
