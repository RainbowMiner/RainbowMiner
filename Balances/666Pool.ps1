...
$Payout_Currencies | Where-Object {-not $Config.ExcludeCoinsymbolBalances.Count -or $Config.ExcludeCoinsymbolBalances -notcontains "$($_.Name)"} | Foreach-Object {
    ...
    try {
        $Request = (Invoke-WebRequestAsync "https://666pool.cn/pool2/main/$($Pool_Currency)/$($Pool_Wallet)" -delay $(if ($Count){1000} else {0}) -cycletime ($Config.BalanceUpdateMinutes*60)).Content
        $Count++
