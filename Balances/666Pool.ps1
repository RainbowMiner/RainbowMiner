...
$Payout_Currencies | Where-Object {-not $Config.ExcludeCoinsymbolBalances.Count -or $Config.ExcludeCoinsymbolBalances -notcontains "$($_.Name)"} | Foreach-Object {
    ...
    try {
        $Request = Invoke-RestMethodAsync "https://666pool.cn/pool/main.php?coin=$($Pool_Currency)&address=$($Pool_Wallet)" -delay $(if ($Count){1000} else {0}) -cycletime ($Config.BalanceUpdateMinutes*60)
        $Count++
