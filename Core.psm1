...
function Stop-Core {
    ...
    if ($IsWindows) {
        ...
        Get-Process | Where-Object Path | Where-Object {$_.Path -like "$(Get-Location)/bin/*"} | Foreach-Object {
            if (Test-OCDaemon) {
                Invoke-Exe "kill" -ArgumentList "$($_.Id)" -RunAs > $null
            } else {
                Stop-Process $_.Id -Force -ErrorAction Ignore
            }
        }
    }
    Stop-Autoexec
