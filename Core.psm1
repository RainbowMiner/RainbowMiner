...
function Invoke-Core {
    ...
    if ($Session.Config.ShowPoolBalances) {
        ...
        if (-not $BalancesData) {$Session.Updatetracker.Balances = 0}
            ...
            if ($RefreshBalances) {$Session.ReportTotals = $true;Start-UpdateTotalsJob}
        }
    }
    ...
    if (-not $Session.IsDonationRun -and -not $Session.Benchmarking -and (-not $Session.Updatetracker.MinerSave -or $Session.Updatetracker.MinerSave -lt (Get-Date).AddHours(-6) -or -not (Test-Path ".\Data\minerdata.json"))) {
        ...
        Set-ContentJson ".\Data\minerdata.json" ([PSObject]@{Miners = @($Miners | Select-Object @{Name="Name";Expression={$_.BaseName}}, Version, @{Name="Algorithm";Expression={$_.BaseAlgorithm | Select-Object -First 1}}, DeviceName, DeviceModel, @{Name="HashRate"; Expression={$_.HashRates.PSObject.Properties.Value | Select-Object -First 1}}, PowerDraw, @{Name="OCProfile"; Expression={if ($Session.Config.EnableOCProfiles -and $_.DeviceModel -ne "CPU" -and $_.DeviceModel -notmatch '-') {$_.OCprofile.PSObject.Properties.Value | Select-Object -First 1} else {""}}} -Unique); OCprofiles=$Session.Config.OCprofiles}) -Compress > $null
