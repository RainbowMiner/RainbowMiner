...
function Invoke-Core {
    ...
    if (-not $Session.IsDonationRun -and -not $Session.Benchmarking -and (-not $Session.Updatetracker.MinerSave -or $Session.Updatetracker.MinerSave -lt (Get-Date).AddHours(-6) -or -not (Test-Path ".\Data\minerdata.json"))) {
        ...
        Set-ContentJson ".\Data\minerdata.json" ([PSObject]@{Miners = @($Miners | Select-Object @{Name="Name";Expression={$_.BaseName}}, Version, @{Name="Algorithm";Expression={$_.BaseAlgorithm | Select-Object -First 1}}, @{Name="DeviceName";Expression={$_.DeviceName -join '-'}}, @{Name="DeviceModel";Expression={if ($_.DeviceModel -eq "CPU") {$Session.DevicesByTypes.CPU | Select-Object -ExpandProperty Model_Name -Unique} else {$_.DeviceModel}}}, @{Name="HashRate"; Expression={$_.HashRates.PSObject.Properties.Value | Select-Object -First 1}}, PowerDraw, @{Name="OCProfile"; Expression={if ($Session.Config.EnableOCProfiles -and $_.DeviceModel -ne "CPU" -and $_.DeviceModel -notmatch '-') {$_.OCprofile.PSObject.Properties.Value | Select-Object -First 1} else {""}}} -Unique); OCprofiles=$Session.Config.OCprofiles}) -Compress > $null
