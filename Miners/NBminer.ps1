...
if ($IsLinux) {
    ...
    $URI = "https://github.com/RainbowMiner/miner-binaries/releases/download/v42.3-nbminer/NBMiner_42.3_Linux.tgz"
} else {
    $Path = ".\Bin\GPU-NBMiner\nbminer.exe"
...
$Commands = [PSCustomObject[]]@(
    ...
    [PSCustomObject]@{MainAlgorithm = "Etchash";      DAG = $true; SecondaryAlgorithm = ""; Params = "-a etchash";       NH = $true; MinMemGb = 3;   DevFee = 1.0; Vendor = @("AMD","NVIDIA");       ExtendInterval = 2; Penalty = 0; NoCPUMining = $false} #Etchash
    [PSCustomObject]@{MainAlgorithm = "Ethash";       DAG = $true; SecondaryAlgorithm = ""; Params = "-a ethash";        NH = $true; MinMemGb = 3;   DevFee = 1.0; Vendor = @("AMD","NVIDIA");       ExtendInterval = 2; Penalty = 0; NoCPUMining = $false} #Ethash
    [PSCustomObject]@{MainAlgorithm = "Ethash2g";     DAG = $true; SecondaryAlgorithm = ""; Params = "-a ethash";        NH = $true; MinMemGb = 1;   DevFee = 1.0; Vendor = @("AMD","NVIDIA");       ExtendInterval = 2; Penalty = 0; NoCPUMining = $false} #Ethash
    [PSCustomObject]@{MainAlgorithm = "Ethash3g";     DAG = $true; SecondaryAlgorithm = ""; Params = "-a ethash";        NH = $true; MinMemGb = 2;   DevFee = 1.0; Vendor = @("AMD","NVIDIA");       ExtendInterval = 2; Penalty = 0; NoCPUMining = $false} #Ethash
    [PSCustomObject]@{MainAlgorithm = "Ethash4g";     DAG = $true; SecondaryAlgorithm = ""; Params = "-a ethash";        NH = $true; MinMemGb = 3;   DevFee = 1.0; Vendor = @("AMD","NVIDIA");       ExtendInterval = 2; Penalty = 0; NoCPUMining = $false} #Ethash
			...
			foreach($MainAlgorithm_Norm in @($MainAlgorithm_Norm_0,"$($MainAlgorithm_Norm_0)-$($Miner_Model)","$($MainAlgorithm_Norm_0)-GPU")) {
				if ($Pools.$MainAlgorithm_Norm.Host -and $Miner_Device -and 
                    ...
                    if ($First) {
                        ...
                        $DeviceIDsAll = ($Miner_Device | % {'{0:d}' -f ($_.Type_Vendor_Index)}) -join ','
					...
					if ($SecondAlgorithm -eq '') {
						...
						[PSCustomObject]@{
							...
							Path           = $Path
							Arguments      = "--api 127.0.0.1:`$mport --platform $(if ($Miner_Vendor -eq "NVIDIA") {1} else {2}) -d $($DeviceIDsAll) -o $($Pool_Protocol)://$($Pools.$MainAlgorithm_Norm.Host):$($Pool_Port) -u $($Pools.$MainAlgorithm_Norm.User)$(if ($Pools.$MainAlgorithm_Norm.Pass) {" -p $($Pools.$MainAlgorithm_Norm.Pass)"})$(if ($FailoverMain) {" $FailoverMain"}) --no-watchdog$(if ($Pools.$MainAlgorithm_Norm.Host -notmatch "Flexpool|Nicehash") {" --share-check 0"}) $($_.Params)"
						...
						[PSCustomObject]@{
							...
							DeviceModel    = $Miner_Model
							Path           = $Path
							Arguments      = "--api 127.0.0.1:`$mport --platform $(if ($Miner_Vendor -eq "NVIDIA") {1} else {2}) -d $($DeviceIDsAll) -o $($Pool_Protocol2)://$($Pools.$SecondAlgorithm_Norm.Host):$($Pool_Port2) -u $($Pools.$SecondAlgorithm_Norm.User)$(if ($Pools.$SecondAlgorithm_Norm.Pass) {" -p $($Pools.$SecondAlgorithm_Norm.Pass)"})$(if ($FailoverSecondary) {" $FailoverSecondary"}) -do $($Pool_Protocol)://$($Pools.$MainAlgorithm_Norm.Host):$($Pool_Port) -du $($Pools.$MainAlgorithm_Norm.User)$(if ($Pools.$MainAlgorithm_Norm.Pass) {" -dp $($Pools.$MainAlgorithm_Norm.Pass)"})$(if ($FailoverMain) {" $FailoverMain"}) -di$($DeviceIntensitiesAll) --no-watchdog --no-health $($_.Params)"
