		...
		foreach($Algorithm_Norm in @($Algorithm_Norm_0,"$($Algorithm_Norm_0)-$($Miner_Model)","$($Algorithm_Norm_0)-GPU")) {
			if ($Pools.$Algorithm_Norm.Host -and $Miner_Device) {
				...
				[PSCustomObject]@{
					...
					Path           = $Path
					Arguments      = "--devices=$($DeviceIDsAll) --address=`"$($Pools.$Algorithm_Norm.Wallet)`" --name=`"$($Pools.$Algorithm_Norm.Worker)`" --server=$($Pools.$Algorithm_Norm.Host) --port=$($Pool_Port) --apiport=$($Miner_Port) --mode=dumb"
