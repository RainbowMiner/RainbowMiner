		    ...
		    foreach($Algorithm_Norm in @($Algorithm_Norm_0,"$($Algorithm_Norm_0)-$($Miner_Model)","$($Algorithm_Norm_0)-GPU")) {
			    if ($Pools.$Algorithm_Norm.Host -and $Miner_Device) {
				    ...
				    [PSCustomObject]@{
					    ...
					    Path           = $Path
					    Arguments      = "$($DeviceParams) -d $($DeviceIDsAll) -a $($Pools.$Algorithm_Norm.Wallet -replace "\s+")$(if ($Pools.$Algorithm_Norm.Pass -and $Pools.$Algorithm_Norm.Pass -ne "x") {" --pool-params $($Pools.$Algorithm_Norm.Pass)"}) -n $($Pools.$Algorithm_Norm.Worker) -p $($Pools.$Algorithm_Norm.Host):$($Pool_Port) --api $($Miner_Port) --mode dumb"
