...
if ($IsLinux) {
    ...
    $Uri = "https://github.com/RainbowMiner/miner-binaries/releases/download/v2.22-gminer/gminer_2_22_linux64.tar.xz"
} else {
    $Path = ".\Bin\GPU-Gminer\miner.exe"
    $Uri = "https://github.com/RainbowMiner/miner-binaries/releases/download/v2.22-gminer/gminer_2_22_windows64.zip"
}
...
$Commands = [PSCustomObject[]]@(
    ...
    [PSCustomObject]@{MainAlgorithm = "Equihash16x5";    MinMemGb = 2;                     Params = "--algo 96_5";        Vendor = @("NVIDIA");       ExtendInterval = 2; AutoPers = $false; ExcludePoolName = "^Zpool"} #Equihash 96,5
    ...
    [PSCustomObject]@{MainAlgorithm = "Ethash";          MinMemGb = 3;                     Params = "--algo ethash";      Vendor = @("NVIDIA");       ExtendInterval = 2; Fee = 0.65; ExcludePoolName = "^(MiningRigRentals|ZergPool)"} #Ethash
    ...
    #[PSCustomObject]@{MainAlgorithm = "Ethash";          MinMemGb = 3;   Intensity = 3;    Params = "--algo ethash+blake2s";        Vendor = @("NVIDIA");       ExtendInterval = 2; Fee = 3.00; SecondaryAlgorithm = "Blake2s"; CoinSymbol2 = @("KDA"); ExcludePoolName = "^ZergPool"; ExcludePoolName2 = "^Nicehash"} #Ethash+Blake2s
    #[PSCustomObject]@{MainAlgorithm = "Ethash";          MinMemGb = 3;   Intensity = 4;    Params = "--algo ethash+blake2s";        Vendor = @("NVIDIA");       ExtendInterval = 2; Fee = 3.00; SecondaryAlgorithm = "Blake2s"; CoinSymbol2 = @("KDA"); ExcludePoolName = "^ZergPool"; ExcludePoolName2 = "^Nicehash"} #Ethash+Blake2s
    #[PSCustomObject]@{MainAlgorithm = "Ethash";          MinMemGb = 3;   Intensity = 5;    Params = "--algo ethash+blake2s";        Vendor = @("NVIDIA");       ExtendInterval = 2; Fee = 3.00; SecondaryAlgorithm = "Blake2s"; CoinSymbol2 = @("KDA"); ExcludePoolName = "^ZergPool"; ExcludePoolName2 = "^Nicehash"} #Ethash+Blake2s
    #[PSCustomObject]@{MainAlgorithm = "Ethash";          MinMemGb = 3;   Intensity = 6;    Params = "--algo ethash+blake2s";        Vendor = @("NVIDIA");       ExtendInterval = 2; Fee = 3.00; SecondaryAlgorithm = "Blake2s"; CoinSymbol2 = @("KDA"); ExcludePoolName = "^ZergPool"; ExcludePoolName2 = "^Nicehash"} #Ethash+Blake2s
    #[PSCustomObject]@{MainAlgorithm = "Ethash";          MinMemGb = 3;   Intensity = 7;    Params = "--algo ethash+blake2s";        Vendor = @("NVIDIA");       ExtendInterval = 2; Fee = 3.00; SecondaryAlgorithm = "Blake2s"; CoinSymbol2 = @("KDA"); ExcludePoolName = "^ZergPool"; ExcludePoolName2 = "^Nicehash"} #Ethash+Blake2s
    #[PSCustomObject]@{MainAlgorithm = "Ethash";          MinMemGb = 3;   Intensity = 8;    Params = "--algo ethash+blake2s";        Vendor = @("NVIDIA");       ExtendInterval = 2; Fee = 3.00; SecondaryAlgorithm = "Blake2s"; CoinSymbol2 = @("KDA"); ExcludePoolName = "^ZergPool"; ExcludePoolName2 = "^Nicehash"} #Ethash+Blake2s
    #[PSCustomObject]@{MainAlgorithm = "Ethash";          MinMemGb = 3;   Intensity = 9;    Params = "--algo ethash+blake2s";        Vendor = @("NVIDIA");       ExtendInterval = 2; Fee = 3.00; SecondaryAlgorithm = "Blake2s"; CoinSymbol2 = @("KDA"); ExcludePoolName = "^ZergPool"; ExcludePoolName2 = "^Nicehash"} #Ethash+Blake2s
    #[PSCustomObject]@{MainAlgorithm = "Ethash";          MinMemGb = 3;   Intensity = 10;   Params = "--algo ethash+blake2s";        Vendor = @("NVIDIA");       ExtendInterval = 2; Fee = 3.00; SecondaryAlgorithm = "Blake2s"; CoinSymbol2 = @("KDA"); ExcludePoolName = "^ZergPool"; ExcludePoolName2 = "^Nicehash"} #Ethash+Blake2s
	...
	$Global:DeviceCache.DevicesByTypes.$Miner_Vendor | Where-Object Type -eq "GPU" | Where-Object {$_.Vendor -ne "NVIDIA" -or $Cuda} | Select-Object Vendor, Model -Unique | ForEach-Object {
        ...
        $Commands.Where({$_.Vendor -icontains $Miner_Vendor -and (-not $_.Version -or [version]$_.Version -le [version]$Version)}).ForEach({
		    ...
		    foreach($MainAlgorithm_Norm in @($MainAlgorithm_Norm_0,"$($MainAlgorithm_Norm_0)-$($Miner_Model)","$($MainAlgorithm_Norm_0)-GPU")) {
			    if ($Pools.$MainAlgorithm_Norm.Host -and $Miner_Device -and (-not $_.ExcludePoolName -or $Pools.$MailAlgorithm_Norm.Name -notmatch $_.ExcludePoolName) -and (-not $_.CoinSymbol -or $_.CoinSymbol -icontains $Pools.$MainAlgorithm_Norm.CoinSymbol) -and (-not $_.ExcludeCoinSymbol -or $_.ExcludeCoinSymbol -inotcontains $Pools.$MainAlgorithm_Norm.CoinSymbol) -and (-not $SecondAlgorithm_Norm -or ($Pools.$SecondAlgorithm_Norm.Host -and (-not $_.ExcludePoolName2 -or $Pools.$SecondAlgorithm_Norm.Name -notmatch $_.ExcludePoolName2) -and (-not $_.CoinSymbol2 -or $_.CoinSymbol2 -icontains $Pools.$SecondAlgorithm_Norm.CoinSymbol) -and (-not $_.ExcludeCoinSymbol2 -or $_.ExcludeCoinSymbol2 -inotcontains $Pools.$SecondAlgorithm_Norm.CoinSymbol)))) {
                    if ($First) {
                    ...
                    if ($SecondAlgorithm_Norm) {
				        ...
				        [PSCustomObject]@{
					        ...
					        DeviceModel    = $Miner_Model
					        Path           = $Path
					        Arguments      = "--api `$mport --devices $($DeviceIDsAll) --server $($Pools.$MainAlgorithm_Norm.Host) --port $($Pool_Port)$(if ($Ethmining -and $Pools.$MainAlgorithm_Norm.EthMode -ne $null -and $Pools.$MainAlgorithm_Norm.EthMode -notin @("ethproxy","qtminer")) {" --proto stratum"}) --user $($Pools.$MainAlgorithm_Norm.User)$(if ($Pools.$MainAlgorithm_Norm.Pass) {" --pass $($Pools.$MainAlgorithm_Norm.Pass)"})$(if ($PersCoin -and ($_.AutoPers -or $PersCoin -ne "auto")) {" --pers $($PersCoin)"})$(if ($Pools.$MainAlgorithm_Norm.SSL) {" --ssl 1"}) --cuda $([int]($Miner_Vendor -eq "NVIDIA")) --opencl $([int]($Miner_Vendor -eq "AMD")) --watchdog 0 --pec 0 --nvml 0 $($_.Params)"
