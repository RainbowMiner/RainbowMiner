		...
		$Commands | ForEach-Object {
			...
			foreach($Algorithm_Norm in @($Algorithm_Norm,"$($Algorithm_Norm)-$($Miner_Model)")) {
				if ($Pools.$Algorithm_Norm.Host -and $Miner_Device) {
                    ...
                    $Miner_Wallet_Params = "-wal $($Pools.$Algorithm_Norm.User)"
					switch($Pools.$Algorithm_Norm.Name) {
                        "F2pool"    {$Miner_Protocol_Params = "";$Miner_Wallet_Params = "-ewal $($Pools.$Algorithm_Norm.User -replace "\..+?$") -worker $($Pools.$Algorithm_Norm.User -replace "^.+?\.")";if ($Pools.$Algorithm_Norm.User -match "^0x[0-9a-f]{40}") {$Pool_Port = 8008}}
					...
					[PSCustomObject]@{
						...
						Path = $Path
						Arguments = "-rmode 0 -cdmport $($Miner_Port) -cdm 1 -log 0 -allpools 0 -leaveoc -coin $(if ($Pools.$Algorithm_Norm.CoinSymbol -eq "UBQ" -or $Pools.$Algorithm_Norm.CoinName -like "ubiq") {"ubq"} else {"auto"}) -di $($DeviceIDsAll) -pool $(if($Pools.$Algorithm_Norm.SSL){"ssl://"})$($Pools.$Algorithm_Norm.Host):$($Pool_Port) $($Miner_Wallet_Params)$(if ($Pools.$Algorithm_Norm.Pass) {" -pass $($Pools.$Algorithm_Norm.Pass)"}) $($Miner_Protocol_Params) $($Miner_Deviceparams) $($_.Params)"
