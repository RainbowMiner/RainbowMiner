...
if ($IsLinux) {
    ...
    $URI = "https://github.com/RainbowMiner/miner-binaries/releases/download/v6.2c-phoenix/PhoenixMiner_6.2c_Linux.tar.gz"
} else {
    $Path = ".\Bin\GPU-Phoenix\PhoenixMiner.exe"
    $URI = "https://github.com/RainbowMiner/miner-binaries/releases/download/v6.2c-phoenix/PhoenixMiner_6.2c_Windows.7z"
}
			...
			foreach($Algorithm_Norm in @($Algorithm_Norm_0,"$($Algorithm_Norm_0)-$($Miner_Model)","$($Algorithm_Norm_0)-GPU")) {
				if ($Pools.$Algorithm_Norm.Host -and $Miner_Device -and (-not $_.ExcludePoolName -or $Pools.$Algorithm_Norm.Host -notmatch $_.ExcludePoolName)) {
					...
					[PSCustomObject]@{
						...
						Path           = $Path
						Arguments      = "-cdmport `$mport -coin $($Coin) -di $($DeviceIDsAll) -pool $(if($Pools.$Algorithm_Norm.SSL){"ssl://"})$($Pools.$Algorithm_Norm.Host):$($Pool_Port) $(if ($Pools.$Algorithm_Norm.Wallet -and $Pools.$Algorithm_Norm.Host -notmatch "gteh|miningrigrentals|nicehash") {"-wal $($Pools.$Algorithm_Norm.Wallet -replace "@","%40") -worker $($Pools.$Algorithm_Norm.Worker)"} else {"-wal $($Pools.$Algorithm_Norm.User -replace "@","%40")"})$(if ($Pools.$Algorithm_Norm.Pass) {" -pass $($Pools.$Algorithm_Norm.Pass)"}) -stales $(if ($Pools.$Algorithm_Norm.Host -match "Nicehash") {0} else {1})$(if ($Pools.$Algorithm_Norm.SSL -and $Pools.$Algorithm_Norm.SSLSelfSigned) {" -weakssl"}) $($Miner_Protocol_Params) $($Miner_Deviceparams) $($CommonParams) $($_.Params)"
