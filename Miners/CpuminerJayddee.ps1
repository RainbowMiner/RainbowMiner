...
if ($IsLinux) {
    ...
    $Uri = "https://github.com/RainbowMiner/miner-binaries/releases/download/v3.16.5-jayddee/cpuminer-opt-3.16.5-linux.7z"
} else {
    ...
    $Uri = "https://github.com/RainbowMiner/miner-binaries/releases/download/v3.16.5-jayddee/cpuminer-opt-3.16.5-windows.zip"
}
...
$Global:DeviceCache.DevicesByTypes.CPU | Select-Object Vendor, Model -Unique | ForEach-Object {
    ...
    $Commands.ForEach({
        ...
        $DeviceParams = "$(if ($CPUThreads){" -t $CPUThreads"})$(if ($CPUAffinity){" --cpu-affinity $CPUAffinity"})$(if ($IsVerthash) {" --data-file '$($VerthashDatFile)'"})"
