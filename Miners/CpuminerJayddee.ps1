...
if ($IsLinux) {
    if ($Global:GlobalCPUInfo.Vendor -eq "ARM" -or $Global:GlobalCPUInfo.Features.ARM) {
        ...
        $Uri  = "https://github.com/RainbowMiner/miner-binaries/releases/download/v26.1-jayddee/cpuminer-opt-26.1-{DISTROCODENAME}-arm.7z" -replace "{DISTROCODENAME}",$distroCodename
        ...
        if ($distroCodename -eq "noble") {
            $Path = ".\Bin\CPU-JayDDee\cpuminer-$($f=$Global:GlobalCPUInfo.Features;$v=$Global:GlobalCPUInfo.Vendor;$(if($f.avx512 -and $f.sha -and $f.vaes) {if ($v -eq "AMD"){"zen4"}else{'avx512-sha-vaes'}}elseif($f.avx512){'avx512'}elseif($f.avx2 -and $f.sha -and $f.vaes) {if ($v -eq "AMD"){"zen3"}else{"avx2-sha-vaes"}}elseif($f.avx2 -and $f.sha -and $f.aes){'avx2-sha'}elseif($f.avx2 -and $f.aes){'avx2'}elseif($f.avx -and $f.aes){'avx'}elseif($f.sse42 -and $f.aes -and $f.sha){'sse42-aes-sha'}elseif($f.sse42 -and $f.aes){'sse42-aes'}elseif($f.sse42){'sse42'}elseif($f.ssse3){"ssse3"}else{'sse2'}))"
        } else {
        ...
        $Uri = "https://github.com/RainbowMiner/miner-binaries/releases/download/v26.1-jayddee/cpuminer-opt-26.1-{DISTROCODENAME}-x64.7z" -replace "{DISTROCODENAME}",$distroCodename
    }
} else {
    ...
    $Uri = "https://github.com/RainbowMiner/miner-binaries/releases/download/v26.1-jayddee/cpuminer-opt-26.1-windows.zip"
}
