...
if ($IsLinux) {
    if ($Global:GlobalCPUInfo.Vendor -eq "ARM" -or $Global:GlobalCPUInfo.Features.ARM) {
        if ($Global:GlobalCPUInfo.Architecture -eq 8) {
            $Path = ".\Bin\CPU-JayDDee\cpuminer-armv8$($f=$Global:GlobalCPUInfo.Features;$(if($f.sha3 -and $f.sve2 -and $f.aes){'-crypto-sha3-sve2'}elseif($f.sha3 -and $f.aes){'-crypto-sha3'}elseif($f.sha2 -and $f.aes){'-crypto'}))"
        ...
        $Uri = "https://github.com/RainbowMiner/miner-binaries/releases/download/v24.4-jayddee/cpuminer-opt-24.4-linux.7z"
    }
} else {
    ...
    $Uri = "https://github.com/RainbowMiner/miner-binaries/releases/download/v24.4-jayddee/cpuminer-opt-24.4-windows.zip"
}
