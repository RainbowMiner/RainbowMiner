...
if ($IsLinux) {
    ...
    $UriCuda = @(
        [PSCustomObject]@{
            Uri = "https://github.com/RainbowMiner/miner-binaries/releases/download/v1.9z5b-miniz/miniZ_v1.9z5b_linux-x64.tar.gz"
            Cuda = "8.0"
        }
    )
} else {
    ...
    $UriCuda = @(
        [PSCustomObject]@{
            Uri = "https://github.com/RainbowMiner/miner-binaries/releases/download/v1.9z5b-miniz/miniZ_v1.9z5b_win-x64.7z"
            Cuda = "8.0"
        }
    )
}
...
$Commands = [PSCustomObject[]]@(
    ...
    [PSCustomObject]@{MainAlgorithm = "UbqHash";       DAG = $true; MinMemGB = 2; Params = "--par=ubqhash";   Vendor = @("AMD","NVIDIA"); ExtendInterval = 3; AutoPers = $false; Fee = 0.75;                  Compute = @("RDNA2")} #Ethash (ETH)
