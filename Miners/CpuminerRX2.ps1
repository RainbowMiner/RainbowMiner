...
$ManualUri = "https://github.com/LUX-Core/rx2-cpuminer/releases"
$Port = "502{0:d2}"
$DevFee = 0.0
$Version = "0.0.1"

if ($IsLinux) {
    #$Path = ".\Bin\CPU-RX2\cpuminer-$($f = $Global:GlobalCPUInfo.Features;$(if($f.avx512){'avx512'}elseif($f.avx2 -and $f.sha -and $f.aes){'zen'}elseif($f.avx2 -and $f.aes){'avx2'}elseif($f.avx -and $f.aes){'aes-avx'}elseif($f.sse42 -and $f.aes){'aes-sse42'}elseif($f.sse42){'sse42'}else{'sse2'}))"
    #$URI = "https://github.com/RainbowMiner/miner-binaries/releases/download/v2.0-take2/cpuminer-take2-ubuntu16-18.tar.gz"
} else {
    $Path = ".\Bin\CPU-RX2\rx2-cpuminer.exe"
    $Uri = "https://github.com/RainbowMiner/miner-binaries/releases/download/v0.0.1-rx2/cpuminer-rx2-0.0.1-win.7z"
}

if (-not $Global:DeviceCache.DevicesByTypes.CPU -and -not $InfoOnly) {return} # No CPU present in system

$Commands = [PSCustomObject[]]@(
    [PSCustomObject]@{MainAlgorithm = "rx2"; Params = ""} #RX2/LUXcoin
)
