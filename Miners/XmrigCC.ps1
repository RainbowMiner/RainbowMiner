...
if ($IsLinux) {
    ...
    if ($Global:GlobalCPUInfo.Vendor -eq "ARM" -or $Global:GlobalCPUInfo.Features.ARM) {
        if ($Global:GlobalCPUInfo.Architecture -eq 8) {
            $Uri = "https://github.com/RainbowMiner/miner-binaries/releases/download/v3.4.2-xmrigcc/xmrigcc-3.4.2-armv8.7z"
        }
    } else {
        if ($Session.LibCVersion -and $Session.LibCVersion -lt (Get-Version "2.25")) {return}

        $Uri = "https://github.com/RainbowMiner/miner-binaries/releases/download/v3.4.2-xmrigcc/xmrigcc-3.4.2-bionic-x64.7z"
