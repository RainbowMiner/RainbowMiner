...
if ($IsLinux) {
    ...
    $URI = "https://github.com/RainbowMiner/miner-binaries/releases/download/v1.5.1-nanominer/nanominer-linux-1.5.1.tar.gz"
} else {
    $Path = ".\Bin\ANY-Nanominer\nanominer.exe"
    $URI = "https://github.com/RainbowMiner/miner-binaries/releases/download/v1.5.1-nanominer/nanominer-windows-1.5.1.zip"
}
...
$Commands = [PSCustomObject[]]@(
    #[PSCustomObject]@{MainAlgorithm = "Cuckaroo29";              Params = ""; MinMemGb = 6; MinMemGbW10 = 8; Vendor = @("AMD","NVIDIA");          NH = $true; ExtendInterval = 2; DevFee = 2.0} #Cuckaroo29
    [PSCustomObject]@{MainAlgorithm = "Cuckarood29";             Params = ""; MinMemGb = 6; MinMemGbW10 = 8; Vendor = @("AMD");          NH = $true; ExtendInterval = 2; DevFee = 2.0} #Cuckaroo29
    ...
    $Session.DevicesByTypes.$Miner_Vendor | Where-Object {$_.Vendor -ne "NVIDIA" -or $Cuda} | Select-Object Vendor, Model -Unique | ForEach-Object {
        ...
        $Commands |  Where-Object {$_.Vendor -icontains $Miner_Vendor} | ForEach-Object {
		    ...
		    foreach($Algorithm_Norm in @($Algorithm_Norm,"$($Algorithm_Norm)-$($Miner_Model)")) {
			    if ($Pools.$Algorithm_Norm.Host -and $Miner_Device -and ($_.NH -or $Pools.$Algorithm_Norm.Name -ne "Nicehash") -and ($Algorithm_Norm -ne "Ethash" -or $Pools.$Algorithm_Norm.Name -notmatch "F2Pool")) {
