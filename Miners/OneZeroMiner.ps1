...
if ($IsLinux) {
    ...
    $Uri = "https://github.com/RainbowMiner/miner-binaries/releases/download/v1.3.3-onezerominer/onezerominer-linux-1.3.3.tar.gz"
} else {
    $Path = ".\Bin\NVIDIA-OneZero\onezerominer.exe"
    $Uri = "https://github.com/RainbowMiner/miner-binaries/releases/download/v1.3.3-onezerominer/onezerominer-win64-1.3.3.zip"
}
		...
		foreach($Algorithm_Norm in @($Algorithm_Norm_0,"$($Algorithm_Norm_0)-$($Miner_Model)","$($Algorithm_Norm_0)-GPU")) {
			if ($Pools.$Algorithm_Norm.Host -and $Miner_Device -and (-not $_.IncludePoolName -or $Pools.$Algorithm_Norm.Host -match $_.IncludePoolName)) {
				...
				[PSCustomObject]@{
					...
					Path           = $Path
					Arguments      = "-d $($DeviceIDsAll) -a $($_.MainAlgorithm) -w $($Pools.$Algorithm_Norm.User)$(if ($Pools.$Algorithm_Norm.Pass) {" -p $($Pools.$Algorithm_Norm.Pass)"}) -o $(if ($Pools.$Algorithm_Norm.SSL) {"$($Pools.$Algorithm_Norm.Protocol)://"})$($Pools.$Algorithm_Norm.Host):$($Pool_Port) --api-port `$mport $($ZilParams)$($_.Params)"
