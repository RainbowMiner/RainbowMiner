    ...
    $Commands | ForEach-Object {
        ...
        if ($Arguments_Platform) {
            if ($_.SecondaryAlgorithm) {
                ...
                $Miner_Name = (@("$($Name)$($SecondaryAlgorithm_Norm)$(if ($_.SecondaryIntensity -ge 0) {$_.SecondaryIntensity})") + @($Miner_Model_Norm) | Select-Object) -join '-'
                ...
                $Miner_Name = (@($Name) + @($Miner_Model_Norm) | Select-Object) -join '-'
            ...
            [PSCustomObject]@{
                ...
                Path        = $Path               
                Arguments   = ("EthDcrMiner64.exe -mport -$($Miner_Port) -epool $($Pools.$MainAlgorithm_Norm.Host):$($Pools.$MainAlgorithm.Port) -ewal $($Pools.$MainAlgorithm_Norm.User) -epsw $($Pools.$MainAlgorithm_Norm.Pass) -allpools 1 -allcoins exp -esm $($EthereumStratumMode)$($Arguments_Secondary)$($_.Params)$($Arguments_Platform) -di $($DeviceIDsAll)" -replace "\s+", " ").trim()
