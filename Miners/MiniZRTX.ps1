...
if ($IsLinux) {
    $Path = ".\Bin\NVIDIA-MiniZRTX\miniZ"
    $UriCuda = @(
        [PSCustomObject]@{
            Uri = "https://github.com/RainbowMiner/miner-binaries/releases/download/v1.6w4-miniz/miniZ_v1.6w4_cuda11_linux-x64.tar.gz"
            Cuda = "11.1"
        }
    )
} else {
    $Path = ".\Bin\NVIDIA-MiniZRTX\miniZ.exe"
    $UriCuda = @(
        [PSCustomObject]@{
            Uri = "https://github.com/RainbowMiner/miner-binaries/releases/download/v1.6w4-miniz/miniZ_v1.6w4_cuda11_win-x64.7z"
            Cuda = "11.1"
        }
    )
}
...
$Global:DeviceCache.DevicesByTypes.NVIDIA | Select-Object Vendor, Model -Unique | ForEach-Object {
    ...
    $Commands.ForEach({
        ...
        $MinMemGB = $_.MinMemGb
        $Miner_Device = $Device | Where-Object {(Test-VRAM $_ $MinMemGB) -and ($_.OpenCL.Architecture -eq "Ampere")}
