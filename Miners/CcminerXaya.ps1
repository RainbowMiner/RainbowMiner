		...
		foreach($Algorithm_Norm in @($Algorithm_Norm_0,"$($Algorithm_Norm_0)-$($Miner_Model)","$($Algorithm_Norm_0)-GPU")) {
			if ($Pools.$Algorithm_Norm.Host -and $Miner_Device) {
                if ($First) {
                    ...
                    $Intensities = @($Miner_Device | Foreach-Object {if ($_.OpenCL.GlobalMemsize -ge 2Gb) {"21"} else {"20"}}) -join ','
                    $First = $false
                }
				...
				[PSCustomObject]@{
					...
					Path           = $Path
					Arguments      = "-R 1 -b `$mport -a $($_.MainAlgorithm) -d $($DeviceIDsAll) -q -o $($Pools.$Algorithm_Norm.Protocol)://$($Pools.$Algorithm_Norm.Host):$($Pool_Port) -u $($Pools.$Algorithm_Norm.User)$(if ($Pools.$Algorithm_Norm.Pass) {" -p $($Pools.$Algorithm_Norm.Pass)"})$(if ($Intensities) {" -i $($Intensities)"}) $($_.Params)"
