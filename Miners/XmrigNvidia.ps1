		...
		foreach($Algorithm_Norm in @($Algorithm_Norm,"$($Algorithm_Norm)-$($Miner_Model)")) {
			if ($Pools.$Algorithm_Norm.Host -and $Miner_Device) {
                ...
                $Arguments = [PSCustomObject]@{
                    ...
                    HwSig   = ($Session.DevicesByTypes.NVIDIA | Where-Object Model -notmatch '-' | Select-Object -Unique -ExpandProperty Model | Sort-Object | Foreach-Object {$m = $_;"$($m)-$(($Session.DevicesByTypes.NVIDIA | Where-Object Model -eq $m | Select-Object -Unique -ExpandProperty Type_Vendor_Index | Sort-Object | % {"{0:x}" -f $_}) -join '')"}) -join '_'
                    Params  =  $Params
                    Threads = 1
                }
