...
if ($IsLinux) {
    ...
    $UriCuda = @(
        [PSCustomObject]@{
            Uri    = "https://github.com/RainbowMiner/miner-binaries/releases/download/v0.18.9-trex/t-rex-0.18.9-linux-cuda11.1.tar.gz"
            Cuda   = "11.1"
            Turing = $true
        },
        [PSCustomObject]@{
            Uri    = "https://github.com/RainbowMiner/miner-binaries/releases/download/v0.18.9-trex/t-rex-0.18.9-linux-cuda10.0.tar.gz"
            Cuda   = "10.0"
            Turing = $true
        },
        [PSCustomObject]@{
            Uri    = "https://github.com/RainbowMiner/miner-binaries/releases/download/v0.18.9-trex/t-rex-0.18.9-linux-cuda9.2.tar.gz"
            Cuda   = "9.2"
        },
        [PSCustomObject]@{
            Uri    = "https://github.com/RainbowMiner/miner-binaries/releases/download/v0.18.9-trex/t-rex-0.18.9-linux-cuda9.1.tar.gz"
            Cuda   = "9.1"
        }
    )
} else {
    ...
    $UriCuda = @(
        [PSCustomObject]@{
            Uri    = "https://github.com/RainbowMiner/miner-binaries/releases/download/v0.18.9-trex/t-rex-0.18.9-win-cuda11.1.zip"
            Cuda   = "11.1"
            Turing = $true
        },
        [PSCustomObject]@{
            Uri    = "https://github.com/RainbowMiner/miner-binaries/releases/download/v0.18.9-trex/t-rex-0.18.9-win-cuda10.0.zip"
            Cuda   = "10.0"
            Turing = $true
        },
        [PSCustomObject]@{
            Uri    = "https://github.com/RainbowMiner/miner-binaries/releases/download/v0.18.9-trex/t-rex-0.18.9-win-cuda9.2.zip"
            Cuda   = "9.2"
        },
        [PSCustomObject]@{
            Uri    = "https://github.com/RainbowMiner/miner-binaries/releases/download/v0.18.9-trex/t-rex-0.18.9-win-cuda9.1.zip"
            Cuda   = "9.1"
        }
    )
}
		...
		foreach($Algorithm_Norm in @($Algorithm_Norm_0,"$($Algorithm_Norm_0)-$($Miner_Model)","$($Algorithm_Norm_0)-GPU")) {
            if ($Pools.$Algorithm_Norm.Host -and $Miner_Device -and (-not $_.ExcludePoolName -or $Pools.$Algorithm_Norm.Name -notmatch $_.ExcludePoolName)) {
				...
				[PSCustomObject]@{
					...
					Path           = $Path
					Arguments      = "-N 10 -r 5 -b 127.0.0.1:`$mport -d $($DeviceIDsAll) -a $($Algorithm) -o $($Pool_Protocol)://$($Pools.$Algorithm_Norm.Host):$($Pool_Port) -u $($Pools.$Algorithm_Norm.User)$(if ($Pools.$Algorithm_Norm.Wallet -and $Pools.$Algorithm_Norm.Worker) {" -w $($Pools.$Algorithm_Norm.Worker)"})$(if ($Pools.$Algorithm_Norm.Pass) {" -p $($Pools.$Algorithm_Norm.Pass)"})$($Pools.$Algorithm_Norm.Failover | Select-Object | Foreach-Object {" -o $($_.Protocol)://$($_.Host):$($_.Port) -u $($_.User)$(if ($_.Pass) {" -p $($_.Pass)"})"})$(if (-not $Session.Config.ShowMinerWindow){" --no-color"}) --no-nvml --no-watchdog --gpu-report-interval 25 --api-bind-http 0 $($_.Params)"
