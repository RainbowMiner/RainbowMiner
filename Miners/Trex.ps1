...
if ($IsLinux) {
    ...
    $UriCuda = @(
        [PSCustomObject]@{
            Uri    = "https://github.com/RainbowMiner/miner-binaries/releases/download/v0.24.5-trex/t-rex-0.24.5-linux.tar.gz"
            Cuda   = "9.2"
        }
    )
} else {
    ...
    $UriCuda = @(
        [PSCustomObject]@{
            Uri    = "https://github.com/RainbowMiner/miner-binaries/releases/download/v0.24.5-trex/t-rex-0.24.5-win.zip"
            Cuda   = "9.2"
        }
    )
}
...
$Commands = [PSCustomObject[]]@(
    ...
    #[PSCustomObject]@{MainAlgorithm = "ethash"; SecondAlgorithm = "autolykos2"; DAG = $true; Params = ""; MinMemGB = 8; ExtendInterval = 3; NoMemCalcCheck = $true} #Ethash+Autolycos2/ERG (new with 0.24.1)
    #[PSCustomObject]@{MainAlgorithm = "ethash"; SecondAlgorithm = "kawpow"; DAG = $true; Params = ""; MinMemGB = 8; ExtendInterval = 3} #Ethash+KawPow/RVN (new with 0.24.1)
    #[PSCustomObject]@{MainAlgorithm = "ethash"; SecondAlgorithm = "octopus"; DAG = $true; Params = ""; MinMemGB = 10; ExtendInterval = 3; NoMemCalcCheck = $true} #Ethash+Octopus/CFX (new with 0.24.1)
...
$Global:DeviceCache.DevicesByTypes.NVIDIA | Select-Object Vendor, Model -Unique | ForEach-Object {
    ...
    $Commands.ForEach({
        ...
        $MinMemGB = if ($_.DAG -and -not $_.NoMemCalcCheck) {Get-EthDAGSize -CoinSymbol $Pools.$Algorithm_Norm_0.CoinSymbol -Algorithm $Algorithm_Norm_0 -Minimum $_.MinMemGb} else {$_.MinMemGb}

        #Zombie-mode since v0.18.3
        if ($_.DAG -and -not $_.NoMemCalcCheck -and $Algorithm_Norm_0 -match $Global:RegexAlgoIsEthash -and $MinMemGB -gt $_.MinMemGB -and $Session.Config.EnableEthashZombieMode) {
            $MinMemGB = $_.MinMemGB
        }
