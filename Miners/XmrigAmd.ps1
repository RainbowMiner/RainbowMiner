		...
		foreach($Algorithm_Norm in @($Algorithm_Norm,"$($Algorithm_Norm)-$($Miner_Model)")) {
			if ($Pools.$Algorithm_Norm.Host -and $Miner_Device) {
				...
				[PSCustomObject]@{
					...
					Path      = $Path
					Arguments = "-R 1 --api-port $($Miner_Port) -a $($xmrig_algo) -o $($Pools.$Algorithm_Norm.Protocol)://$($Pools.$Algorithm_Norm.Host):$($Pool_Port) -u $($Pools.$Algorithm_Norm.User)$(if ($Pools.$Algorithm_Norm.Pass) {" -p $($Pools.$Algorithm_Norm.Pass)"}) --opencl-platform=$($Miner_PlatformId) --opencl-devices=$($DeviceIDsAll) --keepalive$(if ($Pools.$Algorithm_Norm.Name -eq "Nicehash") {" --nicehash"})$(if ($Pools.$Algorithm_Norm.SSL) {" --tls"}) --donate-level=$(if ($IsLinux) {1} else {0}) $($Params)"
