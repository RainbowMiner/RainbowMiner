...
function Invoke-Core {
    ...
    #Load the config    
    $ConfigBackup = if ($Session.Config -is [object]){[PSMemSafeOps]::CopyObject($Session.Config)}else{$null}
    $CheckConfig = $true
    $CheckPools = $false
    ...
    if ($Session.Timer.AddHours(-$DonateDelayHours).AddMinutes($DonateMinutes) -ge $Session.LastDonated -and $Session.AvailPools.Count -gt 0) {
        if ($Session.RoundCounter -gt 0 -and -not $Session.IsDonationRun -and -not $Session.UserConfig) {
            ...
            $Session.Config = $null
            $Session.Config = [PSMemSafeOps]::CopyObject($Session.UserConfig)
    ...
    $Test_Algorithm = $null
    $Test_ExcludeAlgorithm = $null
    $Test_CoinSymbol = $null
    $Test_ExcludeCoinSymbol = $null
    Remove-Variable -Name Test_Algorithm, Test_ExcludeAlgorithm, Test_CoinSymbol, Test_ExcludeCoinSymbol -ErrorAction Ignore
