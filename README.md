...
###### **Central monitoring for all of your rigs: **
...
-   **Fine-tune miner- and pool-configuration during runtime**
...
-   **Central monitoring of all your rigs at **
-   **Offline notifications via email and/or **

## REQUIRED PRE-REQUISITES

### Windows 7/8.1/10 pre-requisites

1.  Install PowerShell 7: [Download Installer for version 7.2.0](https://github.com/PowerShell/PowerShell/releases/download/v7.2.0/PowerShell-7.2.0-win-x64.msi)
...
Find the matching version of [Powershell Core v7.2.0 for your system from here](https://github.com/PowerShell/PowerShell/releases) and download it to your machine.
...
On Linux: either use the GUI installer to install the package or do it manually, e.g. for Ubuntu:
    ...
    wget https://github.com/PowerShell/PowerShell/releases/download/v7.2.0/powershell_7.2.0-1.ubuntu.18.04_amd64.deb

    # install powershell package
    dpkg -i powershell_7.2.0-1.ubuntu.18.04_amd64.deb
...
-   enter API port for webinterface at 
...
-   Find a lot of additional information and documentation here: 
...
If "EnableServerConfig" is set to "1", the client will try to download the config files specified with "ServerConfigName" from the server. If you want to provide the individual rig with specific config files, put them into a subdirectory `.\Config\` (linux: `./Config/`) . Use lowercase letters for the subdirectoy ``.
...
Notes for [NiceHash](https://www.nicehash.com/?refby=c402ea4d-9203-414c-b96e-526e34ad20e1)


Notes for the pools BlockCruncher, BlockMasters, Bsod, CryptoKnight, GosCx, Hashcryptos, HashPool, Icemining, Minerpool, MinerRocks, NLpool, PocketWhale, ProHashing, Ravenminer, YiiMP and Zpool

Notes for [ZergPool](https://zergpool.com/?address=&refcode=78f6ec9427d56069f93709d7805a6a56)

Notes for Solo- and Party-mining
...
| ---------------- | ------ | ------------------------- | ------------------------------------- | ------------------------------------------------------------------ |
| FireIce/XMR-Stak | AMD    | Bin\\Cryptonight-FireIce  | amd.txt                               |  |
| FireIce/XMR-Stak | CPU    | Bin\\Cryptonight-FireIce  | cpu.txt                               |  |
| FireIce/XMR-Stak | NVIDIA | Bin\\Cryptonight-FireIce  | nvidia.txt                            |  |
| JceminerCpu      | CPU    | Bin\\CPU-Jceminer         | config\_[algorithm]-CPU.txt           |                 |
| SrbMiner         | AMD    | Bin\\Cryptonight-Srbminer | config\_[algorithm]-[devicemodel].txt |                 |
...
The new numbers (16x5, 20x9, ..) describe the number of bits matched in each round for the algorithm, and provide a relative measure of the âhardnessâ of the algorithm (the numbers can be calculated, using n and k: n/(k+1) )
...
Open  on your current rig and click "Get setup.json" (or directly enter  into your internet browser) and you will get a setup.json file, containing your current rig's configuration files.
...
**Config files are in JSON format. Look here to get an idea, how they work: **
...
-   **APIPort** = enter a free web-interface port localhost: [default=4000]
...
-   **MinerStatusKey** = your unique miner status key (get one at , you can reuse MPM keys)
-   **MinerStatusEmail** = if your miningrig goes offline at , a warning will be sent to this email-address. Leave empty, if you do not want notifications.
-   **PushOverUserKey** = if your miningrig goes offline at , a notification will be sent via , using your pushover user key. Leave empty, if you do not want this.
...
-   **ExcludeServerConfigVars** = list of parameter names in config.txt or pools.config.txt, that should -not- be overwritten with server values (best is to leave it as is).
    ...
    -   `pools:` = protect all values of pool "poolname"
    -   `pools::` = protect "variablename" in pool "poolname"
...
The line "BTC": "$Wallet" defines the payout wallet address. $Wallet uses the value in your config.txt, $ uses the value, defined for the corresponding currency in coins.config.txt
...
-   define a global RVN wallet with value \. Every occurence of "$RVN" in pools.config.txt will be automatically substituted with this wallet.
...
-   **Description**: Description for autocreate, %workername% will be substituted with rig's workername. Make sure you add [%workername%] \(including the square brackets!) \[default=Autostart mining with RainbowMiner () on Windows. This rig is idle and will activate itself, as soon, as you rent it. [%workername%]]
...
-   to (dis-)allow the CPU rigs: add `cpu`, e.g. if your workername is "RainbowMiner", use "RainbowMinercpu"
-   to (dis-)allow the all-GPU rig: add ``, e.g. if your workername is "RainbowMiner", use it :)
...
RainbowMiner comes with easy setup configuration. Download a setup.json at  from your current rig, copy it into the new rig's RainbowMiner folder *before first start* (you may easily reset RainbowMiner by simply deleting the complete `".\Config"` folder). Now answer two inputs and off you go.
